...
const Questionary = () => {
    ...
    useEffect(() => {
        const fetchPracticeTypeQuestions = async () => {
            try {
                ...
                const questions = res.data.data.assessment.questions;
    ...
    const questions = [
        {
            question: 'When a customer raises a price objection, what is typically the best first response?',
            answers: [
                'Immediately offer a discount to close the deal',
                'Acknowledge the concern and ask questions to understand the underlying issue',
                'Compare your price to competitors to show value',
                'Explain why your product is worth the premium',
            ],
        },
        {
            question: "What is the most effective way to uncover a customer's true needs?",
            answers: [
                'Present all product features and let them choose',
        ...
        {
            question: 'When should you introduce pricing in a sales conversation?',
            answers: [
                'At the very beginning to qualify the prospect',
                'After establishing value and understanding their needs',
                'Only when the customer asks about it',
                'At the end as a closing technique',
            ],
        },
        {
            question: "What's the best approach when a customer says 'I need to think about it'?",
            answers: [
                'Accept it and follow up in a week',
                'Offer a limited-time discount',
                "Ask what specific concerns they'd like to think through",
    ...
    return (
        <div className="min-h-screen bg-[url('/images/Value-Selling-Assessment.png')] bg-cover bg-center relative">
            ...
            <div className="relative w-full max-w-[655px] mx-auto px-[13px] pb-[50px]">
                {/* Main Card */}
                <div className={`relative rounded-[20px] p-4 md:p-[14px] lg:p-9 backdrop-blur-lg ${isDark ? 'bg-[#131313]' : 'bg-[#FFFFFF]/90'}`}>
                    {/* Back Button */}
                    ...
                    <div className="mb-6 md:mb-8">
                        <div className="flex items-center justify-between mb-2 gap-[12px] flex-col md:flex-row">
                            <div className={`w-full h-1.5 md:h-2 rounded-full overflow-hidden order-2 md:order-1 ${isDark ? 'bg-[#908471]' : 'bg-[#908471]'}`}>
                                <div
                                    className={`h-full transition-all duration-300 rounded-full ${isDark ? 'bg-[#DCD6CE]' : 'bg-[#DCD6CE]'}`}
                    ...
                    {/* Question Section */}
                    <div className={`p-[18px] pb-[26px] md:p-[24px] rounded-[12px] md:rounded-[20px] mb-[30px] md:mb-[48px] ${isDark ? 'bg-[#232323]' : 'bg-[#DCD6CE]'}`}>
                        <div className="mb-[8px] md:mb-[28px]">
                            <p className={`text-[10px] md:text-[14px] font-medium uppercase tracking-[0.5px] mb-[5px] ${isDark ? 'text-[#CECECE]' : 'text-[#757575]'}`}>
                                QUESTION {currentQuestion}
                            </p>
                            <h2 className={`text-[16px] md:text-[22px] font-semibold leading-tight mb-0 ${isDark ? 'text-white' : 'text-[#242424]'}`}>{question}</h2>
                        </div>
                            ...
                            {answers.map((answer, index) => (
                                ...
                                >
                                    <div className="flex items-center gap-[8px]">
                                        ...
                                        <span className={`flex-1 text-[10px] md:text-[14px] leading-[15px] ${isDark ? 'text-white' : 'text-[#000000]'}`}>{answer}</span>
                                    </div>
                                </button>
                            ))}
                        </div>
