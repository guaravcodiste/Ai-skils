...
const SystemCheck = () => {
    ...
    const [micPermissionGranted, setMicPermissionGranted] = useState(false); // NEW: Track permission separately
    const [micAudioDetected, setMicAudioDetected] = useState(false); // NEW: Track actual audio detection
    ...
    useEffect(() => {
        if (micCheckRanRef.current) return;
    ...
    const checkMicrophoneAccess = async () => {
        try {
            ...
            setMicStatus('error');
            if ((error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') && !micErrorShownRef.current) {
                ...
                toast.error('Unable to access microphone. Please check your browser settings and reload.');
            }
        }
    };
    ...
    const checkBrowserCompatibility = () => {
        ...
        const isSupported = hasGetUserMedia && hasAudioContext && hasRAF;
    ...
    const getBrowserName = () => {
        ...
        if (/Edg/.test(ua)) return 'Edge Supported';
        if (/Chrome/.test(ua)) return 'Chrome Supported';
        if (/Firefox/.test(ua)) return 'Firefox Supported';
        if (/Safari/.test(ua)) return 'Safari Supported';
        return 'Unknown';
    };

    // Removed redundant useEffect that was setting micStatus based on audioLevel directly
    ...
    const startMicTest = async () => {
        try {
            ...
            const DETECTION_THRESHOLD = 15; // Threshold for audio detection
            const DETECTION_FRAMES = 10; // Number of frames needed to confirm
            ...
            const updateAudioLevel = () => {
                ...
                if (average > DETECTION_THRESHOLD) {
                    ...
                    detectionCounter = Math.max(0, detectionCounter - 1); // Decay counter
                }

                // Mark as detected after sustained audio
                if (detectionCounter >= DETECTION_FRAMES && !micAudioDetected) {
                    setMicAudioDetected(true);
                    setMicStatus('success');

                    // Optional: Auto-stop after 2 seconds of successful detection
                    setTimeout(() => {
            ...
            return (
                <div className="">
                    <svg width="35" height="35" viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M17.4688 0C14.0138 0 10.6364 1.02452 7.76364 2.94402C4.89092 4.86351 2.65191 7.59175 1.32974 10.7837C0.00756894 13.9757 -0.338371 17.4881 0.335665 20.8767C1.0097 24.2653 2.67344 27.378 5.11649 29.821C7.55953 32.2641 10.6722 33.9278 14.0608 34.6018C17.4494 35.2759 20.9618 34.9299 24.1538 33.6078C27.3458 32.2856 30.074 30.0466 31.9935 27.1739C33.913 24.3011 34.9375 20.9237 34.9375 17.4688C34.9326 12.8373 33.0906 8.39685 29.8156 5.12188C26.5407 1.84691 22.1003 0.00489095 17.4688 0ZM25.1382 14.3882L15.732 23.7945C15.6072 23.9194 15.459 24.0185 15.2958 24.0861C15.1327 24.1538 14.9578 24.1886 14.7813 24.1886C14.6047 24.1886 14.4298 24.1538 14.2667 24.0861C14.1036 24.0185 13.9554 23.9194 13.8306 23.7945L9.7993 19.7632C9.54716 19.5111 9.40551 19.1691 9.40551 18.8125C9.40551 18.4559 9.54716 18.1139 9.7993 17.8618C10.0514 17.6097 10.3934 17.468 10.75 17.468C11.1066 17.468 11.4486 17.6097 11.7007 17.8618L14.7813 20.944L23.2368 12.4868C23.3617 12.3619 23.5099 12.2629 23.673 12.1953C23.8361 12.1278 24.0109 12.093 24.1875 12.093C24.3641 12.093 24.5389 12.1278 24.702 12.1953C24.8651 12.2629 25.0134 12.3619 25.1382 12.4868C25.2631 12.6116 25.3621 12.7599 25.4297 12.923C25.4972 13.0861 25.532 13.2609 25.532 13.4375C25.532 13.6141 25.4972 13.7889 25.4297 13.952C25.3621 14.1151 25.2631 14.2634 25.1382 14.3882Z"
                            fill={isDark ? '#8D8D8D' : '#343330'}
                        />
                    </svg>
                </div>
            );
        ...
        return (
            <div className="flex items-end justify-center gap-1.5 h-12 mt-3">
                {barHeights.map((height, index) => (
                    <div
                        key={index}
                        // className="w-1.5 bg-white rounded-full transition-all duration-100 ease-out"
                        className={`w-1.5 ${isDark ? 'bg-white' : 'bg-black'} rounded-full transition-all duration-100 ease-out`}
    ...
    const isSystemReady = micAudioDetected && networkStatus === 'success' && browserStatus === 'success';
                ...
                <div className="relative w-full max-w-[655px] mx-auto px-[13px] pb-[50px]">
                    ...
                    >
                        ...
                        <div className="mb-8 max-w-[522px] mx-auto">
                            <p className={`text-sm md:text-[22px] font-medium text-center mb-2 ${isDark ? 'text-[#CECECE]' : 'text-[#464646]'}`}>System Check</p>
                            <h2 className={`text-2xl md:text-3xl lg:text-[38px] text-center font-semibold ${isDark ? 'text-white' : 'text-[#000000]'}`}>
                        ...
                        <div className="mb-[36px]">
                            <p className={`text-sm md:text-[22px] font-medium text-center mb-[20px] ${isDark ? 'text-[#CECECE]' : 'text-[#000000]'}`}>
                                {micAudioDetected ? 'Microphone verified!' : 'Click to test your microphone'}
                            </p>
                            <div className="flex justify-center">
                                <button
                                    onClick={handleMicTestClick}
                                    disabled={micAudioDetected}
                                    className={`w-full p-[20px] rounded-[20px] transition-all max-w-[460px] ${micAudioDetected
                                            ? 'bg-green-600 text-white cursor-not-allowed'
                                            : isDark
                                                ? 'bg-[#ffffff] bg-opacity-[8%] text-white hover:bg-opacity-[10%]'
                                                : 'bg-[#ffffff] bg-opacity-[33%] text-black hover:bg-opacity-[80%]'
                                        }`}
                                // className={`w-full p-[20px] rounded-[20px] transition-all max-w-[460px] backdrop-blur-lg ${
                                //     isDark ? 'bg-black text-white hover:bg-gray-900' : 'bg-white text-black hover:bg-gray-800'
                                // }`}
                                >
                                        ...
                                        <div className="flex items-center justify-center gap-3">
                                            {micAudioDetected ? (
                                                <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    <path d="M12 17L8 13L9.4 11.6L12 14.2L18.6 7.6L20 9L12 17Z" fill="#10B981" />
                                                </svg>
                                            ) : (
                                                <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <circle cx="14" cy="14" r="14" fill={isDark ? 'white' : 'black'} />
                                                    <path
                                            ...
                                            <span className="font-semibold text-[20px] text-Black dark:text-white">
                                                {/* {isMicTestActive ? 'Stop Mic Test' : 'Start Mic Test'} */}
                                                {micAudioDetected ? 'Mic Test Passed ✓' : isMicTestActive ? 'Testing...' : 'Start Mic Test'}
                                            </span>
                                        </div>
                                    ...
                                    {isMicTestActive && !micAudioDetected && (
                                        <p className="text-[14px] mt-2 text-center text-black dark:text-white">Speak into your microphone...</p>
                                    )}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 mb-8">
                            {/* Microphone Access */}
                            <div
                                className={`flex items-start gap-4 p-4 rounded-[16px] ${isDark
                                        ? micStatus === 'success'
                                            ? 'bg-[#232323] bg-opacity-60'
                                            : 'bg-[#232323] bg-opacity-60'
                                        : micStatus === 'success'
                                            ? 'bg-[#908471] bg-opacity-[0.16]'
                                            : 'bg-[#908471] bg-opacity-[0.16]'
                                    }`}
                            >
                                <div className={`flex-shrink-0 ${micStatus === 'success' ? 'text-green-500' : micStatus === 'error' ? 'text-red-500' : 'text-gray-400'}`}>
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className={`text-[12px] ${isDark ? 'text-white' : 'text-[#000000]'}`}>
                                        {micStatus === 'success'
                                            ...
                                            : micStatus === 'granted'
                                                ? 'Permission granted, please run mic test'
                                                : micStatus === 'pending'
                            ...
                            >
                                <div
                                    className={`flex-shrink-0 ${networkStatus === 'success' ? 'text-green-500' : networkStatus === 'pending' ? 'text-gray-400' : 'text-red-500'
                                        }`}
                                >
                            ...
                            >
                                <div
                                    className={`flex-shrink-0 ${browserStatus === 'success' ? 'text-green-500' : browserStatus === 'pending' ? 'text-gray-400' : 'text-yellow-500'
                                        }`}
                                >
                        ...
                        <div className="flex flex-col gap-1 mb-8">
                            <div className="flex justify-center items-center">
                                ...
                                <h3 className={`font-medium text-[15px] ${isDark ? 'text-white' : 'text-white'}`}>Privacy Protected</h3>
                            </div>
                            <div>
                                <p className={`font-light text-[15px] text-center ${isDark ? 'text-white' : 'text-white'}`}>No recordings retained. Practice is safe.</p>
                            </div>
                        </div>
